...
def scan():
    ...
    if file:
        ...
        file.save(filepath)
        result = fetch_analysis.scan(filepath, filename)
        if result['mismatch'] and flask.request.form.get('virustotal') == 'true':
            virustotal = fetch_virustotal.scan(filepath)
        ...
        if 'user_id' in flask.session:
            ...
            if result['filetype'] == 'UNKNOWN':
                ...
                summary = "Clean"
            database.execute('INSERT INTO logs (user_id, filename, filetype, status) VALUES (?, ?, ?, ?)',
                       (flask.session['user_id'], filename, result['filetype'], summary))
            database.commit()
